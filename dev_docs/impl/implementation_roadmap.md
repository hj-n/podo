...
- turn 종료 시 capture entrypoint를 호출하는 `product/.codex/hooks.json`을 만든다.
- `product/.podo/`의 디렉터리 구조를 만든다.
- Codex runtime 버전별 transcript adapter 계약과 unknown-version 실패 규칙을 정의한다.
...
- `product/.codex/hooks.json`을 설치하고 사용자가 hook을 검토·신뢰하도록 안내한다.
- `.podo/` 제품 영역을 설치한다.
...
- synthetic turn으로 hook과 capture entrypoint가 실제 실행되는지 검증한다.
...
실제 사용에서 확인된 capture, 추적성, 저장 용량과 Context 중복 문제를 해결하고, State와 분리된 People·Research 영역을 추가해 Podo가 사람과 논문 맥락을 지속적으로 관리하게 한다.

### Development Sequence

```text
9.0 Architecture와 데이터 계약
  ↓
9.1 현재 Codex runtime capture 정상화
  ↓
9.2 Markdown 링크, TODO 조회와 중복 탐지
  ↓
9.3 무손실 Event 중복 제거와 Workspace migration
  ↓
9.4 People
  ↓
9.5 Research와 PDF
...
9.6 통합 dogfooding
  ↓
9.7 안정화와 제품 candidate
```

### Work

- 현재 Codex runtime transcript를 fail-closed adapter로 지원하고 capture 상태를 task 시작 시 드러낸다.
- 새 추적 경로를 Markdown 링크로 강제하고 기존 일반 텍스트 경로와 깨진 링크를 읽기 전용으로 발견한다.
- State가 소유하는 TODO를 한 번에 읽는 `podo todos` view를 제공한다.
- 여러 현재 문서에 반복되거나 충돌할 가능성이 있는 Context를 보고하되 자동 병합하지 않는다.
- 누적 transcript snapshot을 byte 단위로 동일하게 복원할 수 있는 content-addressed storage로 중복 제거한다.
- migration은 preview, backup, hash 검증, rollback과 별도 cleanup 승인을 따른다.
- `people/`을 사람별 관계 Context의 별도 사용자 영역으로 구현한다.
- `research/`를 논문 원본, 논문 notes와 주제·프로젝트별 종합의 별도 사용자 영역으로 구현한다.
- PDF는 local-only로 import하고 hash 중복 감지, 원본 추적, 추출 한계와 prompt injection 경계를 적용한다.
- State, People과 Research는 주된 소유 위치를 정하고 Markdown 링크로 연결한다.
...
- 같은 사람이 여러 파일로 갈라지거나 잘못 연결되는지
- 논문이 잘못된 주제·프로젝트에 연결되는지
- 논문 원문과 Podo의 해석을 구분할 수 있는지
...
synthetic Workspace에서 capture, migration, People, Research, PDF와 recovery 시나리오가 반복 통과한다. 사용자가 지정한 실제 Workspace에는 별도 승인된 product update와 migration으로만 적용하며, 실제 사용에서 Podo가 과거 설명 비용을 줄이는지 확인한다.
