...
Podo 제품은 `AGENTS.md`와 `.podo/`로 구성한다.
...
`realpodo`의 변경은 Podo User Workspace에 자동으로 반영되지 않는다. 사용자가 업데이트를 요청하면 변경 내용과 영향을 확인한 뒤 `AGENTS.md`와 `.podo/`만 갱신하고, 설치된 제품 버전을 기록한다.

초기 개발 중에는 로컬 `realpodo`를 업데이트 원본으로 사용할 수 있다. 이후에는 GitHub에 배포한 특정 버전을 원본으로 사용할 수 있다. 원본 위치와 관계없이 같은 install/update 절차를 사용한다.

사용자는 제품 파일을 직접 수정하기보다 `user_config.md`를 통해 Podo를 개인화한다. `AGENTS.md`나 `.podo/`가 직접 수정된 경우에는 업데이트가 이를 조용히 덮어쓰지 않고 충돌을 알려야 한다.
...
Interface Codex는 일반적인 Podo 운영 중 `AGENTS.md`나 `.podo/`를 수정하지 않는다. 제품 파일은 사용자가 명시적으로 Podo 업데이트를 요청했을 때만 정해진 update 절차를 통해 변경한다.
...
| `AGENTS.md`, `.podo/` 변경 | 제품 update 절차 필요 |
...
제품이 소유하는 영역은 `AGENTS.md`와 `.podo/`다. 사용자 소유 영역은 `WORKSPACE_VERSION`, `.podo-work/`, `.podo-backups/`, `user_config.md`, `events/`, `deltas/`와 `state/`다. 제품 update는 사용자 소유 영역을 덮어쓰지 않는다. `WORKSPACE_VERSION`과 `.podo-backups/`는 별도로 승인된 migration과 복구 절차에서만 변경한다.
...
설치 과정은 GitHub에서 최신 안정 버전을 내려받고, 지정한 위치에 `AGENTS.md`와 `.podo/`를 설치한다. `WORKSPACE_VERSION`, `.podo-work/`, `.podo-backups/`, `user_config.md`, `events/`, `deltas/`와 `state/`는 없을 때만 초기 값과 디렉터리를 생성한다. 생성된 뒤에는 사용자 소유가 되며 이후 제품 update가 덮어쓰지 않는다.
...
현재 사용자 데이터와 호환되는지 확인
        ↓
AGENTS.md와 .podo/를 하나의 제품 단위로 교체
        ↓
...
현재 `AGENTS.md`나 `.podo/`가 설치된 버전과 다르면 조용히 덮어쓰지 않는다. 어떤 제품 파일이 달라졌는지 알리고 update를 중단하여 사용자가 보존, 되돌리기 또는 명시적인 교체를 선택할 수 있게 한다.
