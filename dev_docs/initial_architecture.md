...
- `research/`: 논문 원본, 논문별 이해와 주제·프로젝트별 연구 맥락
...
일반 제품 업데이트와 사용자 데이터 migration을 구분한다. 일반 업데이트는 사용자 소유 파일을 수정하지 않는다. `state/`, `people/`, `research/`, `events/` 또는 `deltas/`의 구조를 변경해야 한다면 변경 이유와 영향을 설명하고, 백업과 사용자 확인을 거치는 별도 migration으로 처리한다.
...
## 7. Event, Delta, State, People, and Research Structure

Event, Delta, State, People과 Research는 같은 내용을 반복하지 않고 서로 다른 역할을 가진다.

```text
Event = 무슨 일이 있었는가
Delta = 그래서 무엇이 달라졌는가
State = 지금 무엇이 유효한가
People = 지금 이 사람과의 어떤 관계 맥락이 유효한가
Research = 지금 논문과 연구 주제·프로젝트를 어떻게 이해하는가
```

사람이 직접 읽을 수 있는 파일을 사용한다. Event와 Delta는 발생한 연도와 월로 나누고, State는 오래 유지되는 주제나 프로젝트 이름을 사용한다. People은 사람별 Markdown 파일, Research는 논문별 디렉터리와 주제·프로젝트별 Markdown 파일을 사용한다.
...
Research가 논문 PDF의 정본을 `research/papers/<paper>/original.pdf`로 소유하는 경우에는 같은 PDF를 Event에 다시 복사하지 않는다. 이때 Event Metadata의 `Original-Entrypoint`는 Research의 정본을 가리키고 `SHA-256`으로 동일성을 검증한다. 대화 transcript처럼 별도 사용자 자료 영역이 소유하지 않는 원본은 계속 Event 내부에 보존한다.
...
People은 State의 한 종류가 아니라 사람을 중심으로 현재의 관계 맥락을 관리하는 별도 사용자 영역이다.
...
사람 파일에는 이름과 별칭, 사용자와의 관계, 현재 맥락, 함께 진행하는 일, 약속과 후속 행동 및 관련 State·Delta 링크를 자유로운 Markdown으로 기록한다. 고정된 인맥 category나 심리 프로필을 강제하지 않는다. TODO의 정본은 관련 State에 두고 People에서는 링크하여 중복을 피한다.

동명이인이나 별칭이 모호하면 기존 사람을 임의로 선택하지 않는다. 제3자의 민감한 정보와 Podo가 추론한 성격·의도는 더 보수적으로 다루며 사용자 사실처럼 확정하지 않는다.

### Research

Research는 State의 한 종류가 아니라 논문 원본과 연구 지식을 관리하는 별도 사용자 영역이다.
...
`papers/`는 논문 원본과 논문 자체에 대한 현재 이해를, `topics/`는 여러 논문의 주제별 종합을, `projects/`는 특정 프로젝트에 연구를 적용하는 맥락을 관리한다. 하나의 논문은 여러 주제와 프로젝트에 연결할 수 있으며 하나의 고정 category로 제한하지 않는다.

논문 notes에는 서지정보, 핵심 주장, 방법과 데이터, 결과, 한계, 질문, 프로젝트 관련성과 가능한 경우 페이지 근거를 기록한다. 저자의 주장, 사용자의 판단과 Podo의 추론을 구분한다. 정확한 근거가 필요하면 Research notes가 아니라 PDF 원문을 확인한다.

사용자가 PDF를 전달하고 정리해 달라고 요청하면 해당 자료를 읽고 Research에 보존하는 승인으로 본다. 민감하거나 제3자의 비공개 자료라면 영구 보존 전에 확인한다. 외부 서지정보 조회나 관련 논문 검색은 별도 외부 자료 접근으로 취급한다.
...
Delta에는 영향을 받은 State, People 또는 Research 경로를 기록한다. 하나의 Delta가 여러 파일에 영향을 주면 각 경로를 Markdown 링크로 기록한다. Knowledge Graph나 별도 데이터베이스를 만들지 않는다.
...
일반적인 대화에서는 요청에 맞는 현재 영역을 먼저 읽는다. 계획과 TODO는 State, 사람 중심 맥락은 People, 논문과 연구 질문은 Research에서 시작한다. 변경 이유가 필요할 때 연결된 Delta를 읽으며, 정확한 근거가 필요할 때 Event 또는 Research PDF 원본으로 이동한다. 오래된 자료는 경로, Markdown 링크, Metadata의 내용과 빠른 텍스트 검색으로 찾는다.
...
제품이 소유하는 영역은 `AGENTS.md`, `.codex/hooks.json`과 `.podo/`다. 사용자 소유 영역은 `WORKSPACE_VERSION`, `.podo-work/`, `.podo-backups/`, `user_config.md`, `events/`, `deltas/`, `state/`, `people/`과 `research/`다. 제품 update는 사용자 소유 영역을 덮어쓰지 않는다. `WORKSPACE_VERSION`과 `.podo-backups/`는 별도로 승인된 migration과 복구 절차에서만 변경한다.
...
설치 과정은 GitHub에서 최신 안정 버전을 내려받고, 지정한 위치에 `AGENTS.md`, `.codex/hooks.json`과 `.podo/`를 설치한다. `WORKSPACE_VERSION`, `.podo-work/`, `.podo-backups/`, `user_config.md`, `events/`, `deltas/`, `state/`, `people/`과 `research/`는 없을 때만 초기 값과 디렉터리를 생성한다. 생성된 뒤에는 사용자 소유가 되며 이후 제품 update가 덮어쓰지 않는다.
...
WORKSPACE_VERSION
→ State, People, Research, Event, Delta 등 사용자 데이터의 형식 버전
```
...
- Delta가 존재하는 Event와 State, People 또는 Research를 참조한다.
- State, People과 Research의 중요한 Delta 링크가 실제로 존재한다.
- TODO 날짜 형식이 유효하다.
...
`doctor`는 기본적으로 읽기 전용이다. 파일과 디렉터리 구조, 깨진 링크, Markdown이 아닌 추적 경로, 중복 Context 후보, 누락된 Event·Research 원본, 남아 있는 transaction, capture 상태, 제품 및 Workspace 버전 호환성과 직접 수정된 제품 파일을 확인한다. 문제를 발견해도 사용자 데이터를 자동 수정하지 않는다.
