...
- 실제 사용자 State, People, Research, Event, Delta, transcript, PDF, config와 backup을 이 저장소에 복사하지 않는다.
...
- 사용자 소유 영역은 `WORKSPACE_VERSION`, `.podo-work/`, `.podo-backups/`, `user_config.md`, `events/`, `deltas/`, `state/`, `people/`과 `research/`다.
- 일반 product update는 사용자 소유 파일을 덮어쓰지 않는다.
- 사용자 데이터 변경은 합의된 Context update 또는 migration 흐름을 따른다.
- Context의 핵심 흐름은 `Event → Delta → State / People / Research`다.
- Context를 복원할 때는 요청에 맞는 State, People 또는 Research를 먼저 읽고, 필요한 경우에만 Delta와 Event·PDF를 읽는다.
- State, People과 Research는 실제로 영향을 받은 부분만 수정한다.
...
- Phase 1–9 전체 synthetic regression: `python3 tests/run_phase9_regression.py`
- Phase 9 capture·integrity·Event storage·migration·People·Research 통합 검증: `python3 tests/run_phase9_suite.py`
- Phase 9 실제 Codex People·Research continuity acceptance: `python3 tests/run_phase9_codex_acceptance.py`
- Phase 1–8 전체 synthetic regression: `python3 tests/run_phase8_regression.py`
- Phase 8 install·Context·recovery·product lifecycle 반복 통합 검증: `python3 tests/run_phase8_suite.py`
- Phase 8 실제 Codex 16-task end-to-end acceptance: `python3 tests/run_phase8_codex_acceptance.py`
- Phase 7 migration·backup·full rollback 전체 검증: `python3 tests/run_phase7_suite.py`
- Phase 7 실제 Codex migration 승인 acceptance: `python3 tests/run_phase7_codex_acceptance.py`
