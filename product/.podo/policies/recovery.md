...
`podo recover`로 계획을 만드는 것까지는 읽기 전용이다. State, Event, Delta, receipt, capture, deferred 또는 transaction을 변경하거나 삭제하는 plan은 영향받는 현재 Context를 설명하고 사용자 확인을 받은 뒤 exact plan ID로만 적용한다.
