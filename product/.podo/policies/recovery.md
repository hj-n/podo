...
`podo inbox --json`의 `recovery_required`, `product_recovery_required` 또는 `migration_recovery_required`가 비어 있지 않으면 새 pending Context를 apply하지 않는다. 이때 같은 JSON의 `recovery_diagnosis`는 inbox가 자동 실행한 read-only doctor 결과다. 그 결과를 보기 전에는 진단을 추측해 답하지 않는다. 현재 State가 안전한지와 어느 단계가 미완료인지 사용자에게 high-level로 설명한다. 필요하면 `./.podo/bin/podo doctor --json`으로 같은 진단을 다시 실행할 수 있다.
...
Workspace migration journal이 남아 있으면 제품만 update하거나 backup을 삭제하지 않는다. 일반 Context `podo recover` plan이 migration을 대신 복원한다고 가정하지 않고, journal이 가리키는 source/safety backup과 현재 product/Workspace version을 보존한 채 진단을 설명한다.
