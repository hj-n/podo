#!/usr/bin/env python3
"""Validate an installed Podo User Workspace using its bundled contracts."""
...
VALIDATION_MODES = ("installed-empty", "context-present", "synthetic-fixture")


class Validator:
    def __init__(self, root: Path, mode: str = "context-present"):
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=VALIDATION_MODES, default="context-present")
    args = parser.parse_args()
    validator = Validator(args.workspace, args.mode)
    problems = validator.run()
    if problems:
        for problem in problems:
            print(f"{problem.code} {problem.path}: {problem.message}")
        raise SystemExit(1)
    print(f"OK mode={validator.mode} workspace={validator.root}")
