...
3. `recovery_required` 또는 `product_recovery_required`가 있으면 일반 inbox 처리를 멈춘다. 같은 inbox JSON의 `recovery_diagnosis`가 자동 실행된 read-only doctor 결과이며, `.podo/policies/recovery.md`를 읽어 이 evidence부터 설명한다.
...
- 제품 설치, update 또는 rollback: `.podo/policies/product_update.md`
